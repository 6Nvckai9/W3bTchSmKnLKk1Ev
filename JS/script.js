            ...
            data.forEach(donation => {
                const row = `
                    <tr class="border-b border-gray-700">
                        ...
                        <td class="py-2 px-4 text-[#c99976]">${donation.jumlah}</td>
                    </tr>
                `;
                    ...
                    <div>
                        <div class="text-center my-10" data-aos="fade-up">
                            <h1 class="text-xl sm:text-2xl md:text-3xl font-bold">
                                <span class="text-[#c99976]">${category.category}</span>
                            </h1>
                ...
                category.members.forEach(member => {
                    categoryHTML += `
                        <div class="flex flex-col items-center bg-black/50 p-5 rounded-2xl max-w-xs w-full sm:w-48 shadow-lg hover:shadow-xl transition-all duration-300" data-aos="fade-up">
                            <img src="${member.image}" alt="${member.name}" class="w-16 h-16 sm:w-20 sm:h-20 rounded-full mb-3 border-2 border-[#c99976] shadow-md">
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        leaderboardList.innerHTML = leaderboardData.map(player => `
            <div class="flex items-center gap-4 bg-zinc-800 bg-opacity-90 rounded-xl p-5 w-64 sm:w-72 shadow-lg hover:shadow-xl transition-all border-2 border-[#c99976] ">
